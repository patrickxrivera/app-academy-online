...
def array_sum_with_index(arr)
    arr.each_with_index.inject(0) do |acc, pair|
        num, idx = pair
        acc + (num * idx)
    end 
end
...
def price_is_right(bids, actual_retail_price)
    bids.reduce(0) do |highest_bid_so_far, curr_bid|
        curr_bid <= actual_retail_price ? [highest_bid_so_far, curr_bid].max : highest_bid_so_far
    end
end
...
def products_except_me(numbers)
    product = numbers.reduce { |total, curr| total * curr }
